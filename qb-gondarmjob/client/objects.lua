...
local function GetClosestgondarmObject()
...
RegisterNetEvent('gondarm:client:spawnCone', function()
    QBCore.Functions.Progressbar("spawn_object", "جاري وضعه", 2500, false, true, {
        ...
        TriggerServerEvent("gondarm:server:spawnObject", "cone")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("الغاء", "error")
    end)
end)

RegisterNetEvent('gondarm:client:spawnBarier', function()
    QBCore.Functions.Progressbar("spawn_object", "جاري وضعه", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("gondarm:server:spawnObject", "barier")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("الغاء", "error")
    end)
...
RegisterNetEvent('gondarm:client:spawnSchotten', function()
    QBCore.Functions.Progressbar("spawn_object", "جاري وضعه", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("gondarm:server:spawnObject", "schotten")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("الغاء", "error")
    end)
...
RegisterNetEvent('gondarm:client:spawnTent', function()
    QBCore.Functions.Progressbar("spawn_object", "جاري وضعه", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("gondarm:server:spawnObject", "tent")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("الغاء", "error")
    end)
...
RegisterNetEvent('gondarm:client:spawnLight', function()
    QBCore.Functions.Progressbar("spawn_object", "جاري وضعه", 2500, false, true, {
        disableMovement = true,
        disableCarMovement = true,
        disableMouse = false,
        ...
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        TriggerServerEvent("gondarm:server:spawnObject", "light")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@narcotics@trash", "drop_front", 1.0)
        QBCore.Functions.Notify("الغاء", "error")
    end)
...
RegisterNetEvent('gondarm:client:deleteObject', function()
    local objectId, dist = GetClosestgondarmObject()
    if dist < 5.0 then
        QBCore.Functions.Progressbar("remove_object", "جاري تخزينه", 2500, false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            TriggerServerEvent("gondarm:server:deleteObject", objectId)
        end, function() -- Cancel
            StopAnimTask(PlayerPedId(), "weapons@first_person@aim_rng@generic@projectile@thermal_charge@", "plant_floor", 1.0)
            QBCore.Functions.Notify("الغاء", "error")
        end)
...
RegisterNetEvent('gondarm:client:removeObject', function(objectId)
    NetworkRequestControlOfEntity(ObjectList[objectId].object)
    DeleteObject(ObjectList[objectId].object)
    ObjectList[objectId] = nil
end)

RegisterNetEvent('gondarm:client:spawnObject', function(objectId, type, player)
...
RegisterNetEvent('gondarm:client:SpawnSpikeStrip', function()
    if #SpawnedSpikes + 1 < SpikeConfig.MaxSpikes then
        if PlayerJob.name == "gondarm" and PlayerJob.onduty then
            ...
            TriggerServerEvent('gondarm:server:SyncSpikes', SpawnedSpikes)
        end
    else
        QBCore.Functions.Notify('لم يتبق هناك أي أشواك', 'error')
    end
end)

RegisterNetEvent('gondarm:client:SyncSpikes', function(table)
    SpawnedSpikes = table
end)
            ...
            if ClosestSpike then
                ...
                if dist < 4 then
                    if not IsPedInAnyVehicle(PlayerPedId()) then
                        if PlayerJob.name == "gondarm" and PlayerJob.onduty then
                            sleep = 0
                            DrawText3D(pos.x, pos.y, pos.z, '[~r~E~w~] ﻖﻳﺮﻄﻟﺍ ﻦﻣ ﺔﻟﺍﺯﺍ')
