...
local StatusList = {
    ['fight'] = 'أيادي حمراء',
    ['widepupils'] = 'تلاميذ على نطاق واسع',
    ['redeyes'] = 'عيون حمراء',
    ['weedsmell'] = 'تنبعث منه رائحة الحشيش',
    ['gunpowder'] = 'البارود في الملابس',
    ['chemicals'] = 'رائحة كيميائية',
    ['heavybreath'] = 'يتنفس بشكل كبير',
    ['sweat'] = 'يتعرق كثيرا',
    ['handbleed'] = 'الدم على اليدين',
    ['confused'] = 'خجول',
    ['alcohol'] = 'رائحته مثل الكحول',
    ['heavyalcohol'] = 'رائحته تشبه إلى حد كبير رائحة الكحول'
}
...
RegisterNetEvent('evidence:client:ClearBlooddropsInArea', function()
    ...
    QBCore.Functions.Progressbar('clear_blooddrops', 'Clearing blood..', 5000, false, true, {
        ...
        QBCore.Functions.Notify('الدم لا يطهر', 'error')
    end)
end)
...
RegisterNetEvent('evidence:client:AddCasing', function(casingId, weapon, coords, serie)
    Casings[casingId] = {
        type = weapon,
        serie = serie and serie or 'الرقم التسلسلي غير مرئي',
        coords = {
            x = coords.x,
            y = coords.y,
            z = coords.z - 0.9
...
RegisterNetEvent('evidence:client:ClearCasingsInArea', function()
    local pos = GetEntityCoords(PlayerPedId())
    local casingList = {}
    QBCore.Functions.Progressbar('clear_casings', 'تحصيل الدلائل', 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true
        ...
        QBCore.Functions.Notify('تم ازلة الدليل', 'error')
    end)
end)
    ...
    while true do
        ...
        if CurrentCasing and CurrentCasing ~= 0 then
            ...
            if #(pos -vector3(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z)) < 1.5 then
                DrawText3D(Casings[CurrentCasing].coords.x, Casings[CurrentCasing].coords.y, Casings[CurrentCasing].coords.z, '~g~G~w~ - ﻞﻴﻟﺩ ~b~#' .. Casings[CurrentCasing].type)
        ...
        if CurrentBlooddrop and CurrentBlooddrop ~= 0 then
            ...
            if #(pos - vector3(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y,
                ...
                DrawText3D(Blooddrops[CurrentBlooddrop].coords.x, Blooddrops[CurrentBlooddrop].coords.y, Blooddrops[CurrentBlooddrop].coords.z, '~g~G~w~ - ﻡﺩ ~b~#' .. DnaHash(Blooddrops[CurrentBlooddrop].citizenid))
                if IsControlJustReleased(0, 47) then
        ...
        if CurrentFingerprint and CurrentFingerprint ~= 0 then
            ...
            if #(pos - vector3(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y,
                ...
                DrawText3D(Fingerprints[CurrentFingerprint].coords.x, Fingerprints[CurrentFingerprint].coords.y, Fingerprints[CurrentFingerprint].coords.z, '~g~G~w~ - ﺔﻤﺼﺑ')
                if IsControlJustReleased(0, 47) then
