...
RegisterNetEvent('gondarm:client:ActiveCamera', function(cameraId)
    ...
    elseif cameraId == 0 then
        ...
        QBCore.Functions.Notify("الكاميرا غير موجودة", "error")
    end
end)

RegisterNetEvent('gondarm:client:DisableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('gondarm:client:EnableAllCameras', function()
    for k, v in pairs(Config.SecurityCameras.cameras) do 
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('gondarm:client:SetCamera', function(key, isOnline)
