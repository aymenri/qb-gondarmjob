		...
		if IsPedInAnyVehicle(myPed, false) then
			if GetPedInVehicleSeat(vehicle, -1) == myPed then
				if GetVehicleClass(vehicle) ~= 18 then
					...
					QBCore.Functions.TriggerCallback('gondarm:IsPlateFlagged', function(result)
						if result then
							...
							local blipsettings = {
								...
								color = 1,
								scale = 0.9,
								text = "تم تحديد سرعة سيارة تجاوزت #"..hasBeenBusted..""
							}
							local s1, s2 = GetStreetNameAtCoord(coords.x, coords.y, coords.z)
							local street1 = GetStreetNameFromHashKey(s1)
							local street2 = GetStreetNameFromHashKey(s2)
							TriggerServerEvent("gondarm:server:FlaggedPlateTriggered", hasBeenBusted, plate, street1, street2, blipsettings)
						end
                    end, plate)
				end
			end
